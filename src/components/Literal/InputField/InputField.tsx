...
const RawInput = styled.input.attrs((props: { color: string }) => ({
  type: "text",
  size: "1",
}))`
  grid-area: 1 / 1 / 2 / 2;
...
const RawHiddenSpan = styled.span`
  grid-area: 1 / 1 / 2 / 2;
  visibility: hidden;
  height: 0;
    ...
    <div className="input" ref={setNodeRef} {...attributes} {...listeners}>
      ...
      >
        <Input
          ...
          onBlur={onBlur}
        />
        <HiddenSpan>{props.value === "" ? "_" : props.value}</HiddenSpan>
      </Container>
    </div>
  );
}

export function GrowingInput(props: {
  color: string;
  value: string;
  onFocus: () => void;
  ...
  return (
    <GridContainer>
      ...
      />
      <RawHiddenSpan>{props.value === "" ? "_" : props.value}</RawHiddenSpan>
    </GridContainer>
  );
}
