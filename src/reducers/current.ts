...
export const insertExpressionBlock = createAsyncThunk(
    ...
    (data: { expressionId: ExpressionId, expressionBlockId: ExpressionBlockId }, { getState }) => {
        const state = getState() as State;
        return {
            undo: false,
            name: "Insert Expression Block",
            expressionId: data.expressionId,
            expressionBlockId: data.expressionBlockId,
            replacedExpression: state.current.expressions[data.expressionId],
            parent: state.current.expressionBlocks[data.expressionBlockId].parent
        }
    }
);

export const _insertExpressionBlock = (state: CurrentState, expressionId: ExpressionId, expressionBlockId: ExpressionBlockId, parent: ExpressionId) => {
    ...
    if (parent !== expressionId) {
        ...
        if (state.expressionBlocks[expressionBlockId].type === type) {
            ...
            if (expressionBlock.type in defaultValues) {
                state.expressions[parent] = defaultValues[expressionBlock.type as ValueType];
...
export const _undoInsertExpressionBlock = (state: CurrentState, expressionId: ExpressionId, expressionBlockId: ExpressionBlockId, parent: ExpressionId, replacedExpression: Expression) => {
    const expression = state.expressions[expressionId];

    // If the block has actually been inserted
    if (expression.expressionType === "Block" && expression.expressionBlock === expressionBlockId) {
...
export const current = createReducer(initialCurrentState, (builder) => {
    builder
        .addCase(insertExpressionBlock.fulfilled, (state, { payload: { undo, expressionId, expressionBlockId, replacedExpression, parent }}) => {
            if (undo) {
                _undoInsertExpressionBlock(state, expressionId, expressionBlockId, parent, replacedExpression);
            } else {
                _insertExpressionBlock(state, expressionId, expressionBlockId, parent);
            }
        })
